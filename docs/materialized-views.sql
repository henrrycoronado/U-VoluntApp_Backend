...
SELECT 
    ...
    cs.name AS contract_state,
    us.end_date
FROM public.user_scholarships us
JOIN public.profiles p ON us.assigned_profile_code = p.uva_code
JOIN public.scholarship_type cst ON us.scholarship_type_code = cst.uva_code
JOIN public.contract_state cs ON us.state_code = cs.uva_code
...
LEFT JOIN public.tracking_logs tl ON e.uva_code = tl.enrollment_code 
    AND tl.state_code = (SELECT uva_code FROM public.tracking_state WHERE name = 'active')
    AND tl.created_at >= us.start_date
    AND tl.deleted_at IS NULL
GROUP BY 
    p.uva_code, p.first_name, p.last_name, cst.name, us.required_hours, cs.name, us.end_date, us.uva_code;

CREATE UNIQUE INDEX idx_mv_scholarship_perf_code ON public.mv_scholarship_performance(scholarship_code);

-- Evita duplicados en conteo usando subconsulta GROUP BY enrollment
...
WITH activity_hours AS (
    ...
    LEFT JOIN public.enrollments e ON a.uva_code = e.activity_code 
        AND e.state_code = (SELECT uva_code FROM public.enrollment_state WHERE name = 'active')
...
-- Evita duplicados usando subconsulta para tracking logs agrupados por enrollment
...
WITH enrollment_hours AS (
    ...
    LEFT JOIN public.tracking_logs tl ON e.uva_code = tl.enrollment_code 
        AND tl.state_code = (SELECT uva_code FROM public.tracking_state WHERE name = 'active')
        AND tl.deleted_at IS NULL
    WHERE e.state_code = (SELECT uva_code FROM public.enrollment_state WHERE name = 'active')
        AND e.deleted_at IS NULL
...
-- Sin hardcoded IDs, usa join a profile_state para buscar 'active'
...
SELECT 
    ...
    COALESCE(SUM(CASE WHEN ts.name = 'active' THEN tl.calculated_hours ELSE 0 END), 0) AS validated_hours,
    COALESCE(SUM(CASE WHEN ts.name IN ('active', 'under_review') THEN tl.calculated_hours ELSE 0 END), 0) AS total_logged_hours,
    MAX(tl.created_at) AS last_activity_date
FROM public.profiles p
LEFT JOIN public.career_type cc ON p.career_code = cc.uva_code
LEFT JOIN public.enrollments e ON p.uva_code = e.enrolled_profile_code 
    AND e.state_code = (SELECT uva_code FROM public.enrollment_state WHERE name = 'active')
    AND e.deleted_at IS NULL
LEFT JOIN public.tracking_logs tl ON e.uva_code = tl.enrollment_code AND tl.deleted_at IS NULL
LEFT JOIN public.tracking_state ts ON tl.state_code = ts.uva_code
WHERE p.deleted_at IS NULL
GROUP BY 
    p.uva_code, p.first_name, p.last_name, cc.name, p.personal_goal_hours;
